...
gateway_uuid = "20ce1603-7865-46c3-86dc-a06f4f1b43b8" --enter the UUID of the gateway that should be used for outbound dialing
params = {} --empty table
...
if values_returned == 1 then
	...
	--attempt the call
	outSession = freeswitch.Session("{origination_caller_id_name="..queue["campaign_name"]..",origination_caller_id_number=".. "9057592660" .."}sofia/gateway/".. gateway_uuid .."/".. queue["phone_number"])
	...
	if ( outSession:ready() ) then
		...
		if digits == "1" then
			...
			dbh:query("UPDATE v_dialer_queue SET status = 'complete', result = 'accepted', attempts = "..queue["attempts"]+1 .." where primary_key = "..queue["primary_key"], params);
			...
			dbh:query("UPDATE v_dialer_queue SET result = 'not_accepted', attempts = "..queue["attempts"]+1 .." where primary_key = ".. queue["primary_key"], params);
		end
		outSession:hangup();
	...
	--unable to complete the call
		...
		dbh:query("UPDATE v_dialer_queue SET status = '".. status .."', attempts = ".. queue["attempts"]+1 ..", result = '".. outCause .."' where primary_key = ".. queue["primary_key"], params);
